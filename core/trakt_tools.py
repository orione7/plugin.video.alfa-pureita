...
def auth_trakt():
    ...
    try:
        ...
        if not item.folder:
            ...
            itemlist = []
            title = "Accedi a questa pagina: %s" % item.verify_url
            itemlist.append(item.clone(title=title, action=""))
            title = "Inserisci questo codice e accetta: %s" % item.user_code
            itemlist.append(item.clone(title=title, action=""))
            title = "Una volta fatto, clicca qui!"
...
def token_trakt(item):
    ...
    try:
        if item.extra == "renew":
            ...
            import time
            dialog_auth = platformtools.dialog_progress("Sincronizzare con Trakt. Non chiudere questa finestra",
                                                        "1. Inserisci il seguente URL: %s" % item.verify_url,
                                                        "2. Inserisci questo codice sulla pagina e accetta:  %s"
                                                        % item.user_code,
                                                        "3. Attendi fino a quando questa finestra non si chiude")
        ...
        if not item.folder:
            platformtools.dialog_notification("Successo", "Account collegato correttamente")
            if config.is_xbmc():
                import xbmc
        ...
        logger.error(traceback.format_exc())
        if not item.folder:
            return platformtools.dialog_notification("Error", "Errore nel processo di collegamento")
        token = ""

    itemlist = []
    if token:
        itemlist.append(item.clone("Account collegato correttamente", action=""))
    else:
        itemlist.append(item.clone("Errore nel processo di collegamento", action=""))
...
def ask_install_script():
    ...
    respuesta = platformtools.dialog_yesno("Alfa-PureITA", "Puoi installare lo script Trakt qui sotto, "
                                                   "una volta installato e configurato"
                                                   "si sincronizzerà automaticamente con il tuo account.",
                                           "Desideri Continuare?")
    if respuesta:
